...
import { HOST_DASHBOARD_METRICS, HOST_EVENTS } from '../data/hostEvents';

const approvalQueue = [
  { name: 'Lê Minh Khoa', event: 'Da Nang Open Badminton 2026', time: '2 phút trước', action: 'Xác minh hồ sơ' },
  { name: 'Nguyễn Thu Hà', event: 'Summer Club Cup 2026', time: '14 phút trước', action: 'Xét hạng mục đôi' },
  { name: 'Alex Chen', event: 'Friendly Mix Doubles', time: '1 giờ trước', action: 'Chờ thanh toán' },
];
  ...
  return (
    <div className="space-y-xl animate-fade-in-up">
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-lg">
        {HOST_DASHBOARD_METRICS.map((metric, index) => {
          const href =
            metric.key === 'events'
              ? '/host/tournaments'
              : metric.key === 'pending'
        ...
        <div className="xl:col-span-4 space-y-lg">
          <article className="glass-card rounded-[28px] p-lg">
            <div className="flex items-center justify-between mb-5">
              ...
              <span className="text-xs font-semibold px-2 py-1 rounded-full bg-secondary-container/50 text-secondary">3 mới</span>
            </div>
            <div className="space-y-3">
              {approvalQueue.map((item) => (
                <div key={`${item.name}-${item.event}`} className="rounded-2xl border border-outline-variant/50 bg-white/75 p-4">
